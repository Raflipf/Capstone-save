...
class StorageManager {
    ...
    addToQueue(patient) {
        try {
            ...
            this.setQueue(queue);
            this.addActivity(`Patient ${patient.name} added to queue (#${queueNumber})`);
    ...
    updateQueueStatus(queueId, status) {
        try {
            ...
            if (index !== -1) {
                ...
                this.setQueue(queue);
                this.addActivity(`Queue #${queue[index].queueNumber} status changed from ${oldStatus} to ${status}`);
                return queue[index];
            }
            return null;
        } catch (error) {
    ...
    updateStats() {
        try {
            ...
            const stats = {
                ...
                queueLength: queue.filter(entry => entry.status !== 'completed').length
            };
    ...
    addMedicalRecord(record) {
        try {
            ...
            this.addActivity(`Medical record added for patient ${record.patientName}`);
            return newRecord;
        } catch (error) {
